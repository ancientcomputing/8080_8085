...
sim     .equ    030h    ; ASMX doesn't seem to recognize these 8085 instructions
rim     .equ    020h
bittime .equ    032h    ; 2400 baud for 6.144MHz clock
halfbit .equ    018h    ; 19h
numbits .equ    11      ; Two stop bits
inbits  .equ    9       ; One stop bit

; Check if SID is low
checkline:
        db      rim
        ora     a
        jp      checkline0      ; Line is low
        xra     a               ; Set zero flag
checkline0:                     ; zero flag = 0      
        ret
...
ci3:
        ...
        ral             ; Bit in cy
        dcr     b
        jz      ci5     ; First stop bit, quit
        mov     a,c     ; Rotate partial char in to C
        rar
        mov     c, a
        nop             ; cin and cout loop times are the same
        jmp     ci3
ci5:
        pop     h
        pop     d
...
;       ORG  1366H
;       ORG  1F00H
	ORG  0FE00H			;for 32K RAM (256bytes less than STACK top
...
;       ORG  1400H
;       ORG  2000H
	ORG  0FF00H			;for 64K system -- 32k ROM, 32K RAM
